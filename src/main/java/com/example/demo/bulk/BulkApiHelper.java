...
import com.example.demo.controller.Account2SDFC;
...
//		}
        ...
        System.out.println("account--lists-->" + lists);
        List<Account> accountList = new ArrayList<Account>();
        for (List<String> strList : lists) {

            Account account = new Account();
            account.id = strList.get(0);
            account.Name = strList.get(1);
            account.Phone = strList.get(2);
            accountList.add(account);
        }
        //Account2SDFC acc2SFDC = new Account2SDFC();
        // acc2SFDC.accountAdd(accountList);

    }

    public static List<Account> SFDC2DB() throws AsyncApiException, ConnectionException, IOException, InterruptedException {

        BulkApiHelper bah = new BulkApiHelper(BulkApiHelper.DEFAULT_TEST_ENDPOINT, "zhang520lovelin@163.com", "zhang1111RaaDtQGQmvNGh9NLzVgWU2VZ");
        String soql = "SELECT Id,Name,Phone from Account";
        List<List<String>> lists = bah.query("Account", soql);//lists whose first line as header and the rest as data
        System.out.println("account--lists-->" + lists);
        List<Account> accountList = new ArrayList<Account>();
